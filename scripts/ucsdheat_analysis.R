...
campus <- "ucsd"
campus_name <- "UC San Diego"

# Look for the UC San Diego Landsat folder
...
if (!any(candidate_has_data)) {
  ...
  stop(
    paste0(
      "Could not find the UC San Diego Landsat files.\n",
...
# Find the exact UC San Diego boundary file
...
if (!any(boundary_exists)) {
  ...
  stop(
    paste0(
      "Could not find the UC San Diego boundary file.\n",
...
if (nrow(scene_table) == 0) {
  stop(
    "No complete UC San Diego Landsat scenes were matched."
  )
}
...
ucsd_boundary <- st_read(
  boundary_file,
  quiet = TRUE
)

if (is.na(st_crs(ucsd_boundary))) {
  stop(
    paste0(
      "The UC San Diego boundary file does not ",
...
ucsd_boundary_projected <- st_transform(
  ucsd_boundary,
  crs(reference_temperature)
)

ucsd_boundary_vect <- vect(
  ucsd_boundary_projected
)
...
plot(
  temperature_fahrenheit,
  main = paste0(
    "Median UC San Diego Surface Temperature\n",
    study_start,
    " to ",
    study_end
  )
...
cat(
  "Polished UC San Diego map images saved successfully!\n"
)
...
cat(
  "Transparent UC San Diego web PNGs exported successfully!\n"
)
...
cat(
  "UC San Diego web data CSV exported successfully!\n"
)
...
cat(
  "\nSaved web/data/ucsd_map_metadata.json\n"
)
...
cat(
  "\nAll UC San Diego outputs exported successfully!\n"
)
...
cat(
  "\nUC San Diego files in outputs/data:\n"
)
...
cat(
  "\nUC San Diego files in outputs/figures:\n"
)
...
cat(
  "\nUC San Diego files in web/data:\n"
)
...
cat(
  "\nUC San Diego analysis complete!\n"
)