...
if (!dir.exists(landsat_folder)) {
  ...
  stop(
    paste(
      "The UC Merced Landsat folder does not exist:",
      landsat_folder
    )
  )
}
...
if (!file.exists(boundary_file)) {
  ...
  stop(
    paste(
      "The UC Merced boundary file does not exist:",
      boundary_file
    )
  )
}
...
if (nrow(scene_table) == 0) {
  stop(
    "No complete UC Merced Landsat scenes were matched."
  )
}
...
ucm_boundary <- st_read(
  boundary_file,
  quiet = TRUE
)

if (is.na(st_crs(ucm_boundary))) {
  stop(
    paste0(
      "The UC Merced boundary does not have ",
...
ucm_boundary_projected <- st_transform(
  ucm_boundary,
  crs(reference_temperature)
)

ucm_boundary_vect <- vect(
  ucm_boundary_projected
)
...
plot(
  temperature_fahrenheit,
  main = paste0(
    "Median UC Merced Surface Temperature\n",
    study_start,
    " to ",
    study_end
  )
...
cat(
  "Polished UC Merced map images saved successfully!\n"
)
...
cat(
  "Transparent UC Merced web PNGs exported successfully!\n"
)
...
cat(
  "UC Merced web data CSV exported successfully!\n"
)
...
cat(
  "\nSaved web/data/ucm_map_metadata.json\n"
)
...
cat(
  "\nAll UC Merced outputs exported successfully!\n"
)
...
cat(
  "\nUC Merced files in outputs/data:\n"
)
...
cat(
  "\nUC Merced files in outputs/figures:\n"
)
