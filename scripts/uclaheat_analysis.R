...
##################################
## STEP 2. LOCATE UCLA FILES
##################################

campus <- "ucla"
campus_name <- "UCLA"

# Look for the UCLA Landsat folder in several likely locations
landsat_candidates <- c(
  file.path("landsat", "ucla_landsat"),
  file.path("landsat", "ucladata_landsat"),
  "ucla_landsat",
  "ucladata_landsat"
)

# Choose the first candidate that contains temperature raster files
...
if (!any(candidate_has_data)) {
  ...
  if (dir.exists("landsat")) {
    ...
    print(list.files("landsat"))
  }
  
  stop(
    paste0(
      "Could not find the UCLA Landsat files.\n",
      "Expected one of these folders:\n",
      paste(landsat_candidates, collapse = "\n")
    )
  )
}
...
# Find the exact UCLA boundary file
boundary_candidates <- c(
  file.path("boundary", "UCLA map.kml"),
  file.path("boundary", "ucla.kml"),
  file.path("boundary", "UCLA.kml")
)
...
if (!any(boundary_exists)) {
  ...
  print(list.files("boundary"))
  
  stop(
    paste0(
      "Could not find the UCLA boundary file.\n",
      "Expected one of these files:\n",
      paste(boundary_candidates, collapse = "\n")
    )
  )
}
...
# Extract the satellite, path/row and acquisition date
...
if (any(is.na(temperature_table$scene_id))) {
  stop(
    "At least one temperature filename did not contain a recognizable Landsat scene ID."
  )
}

if (any(is.na(qa_table$scene_id))) {
  stop(
    "At least one QA filename did not contain a recognizable Landsat scene ID."
  )
}

if (any(is.na(red_table$scene_id))) {
  stop(
    "At least one red-band filename did not contain a recognizable Landsat scene ID."
  )
}

if (any(is.na(nir_table$scene_id))) {
  stop(
    "At least one NIR filename did not contain a recognizable Landsat scene ID."
  )
}

# Join files by actual Landsat scene identifier
...
# Check whether any scene is missing a required file
...
if (nrow(incomplete_scenes) > 0) {
  ...
  stop(
    "At least one Landsat scene is missing a required temperature, QA, red or NIR file."
  )
}

if (nrow(scene_table) == 0) {
  stop(
    "No complete Landsat scenes were matched."
  )
}
...
if (is.na(st_crs(ucla_boundary))) {
  stop(
    "The UCLA boundary file does not have a valid coordinate reference system."
  )
}
...
cat(
  "\nBoundary CRS:",
  st_crs(ucla_boundary_projected)$input,
  "\n"
)
...
plot(
  temperature_fahrenheit,
  main = paste0(
    "Median UCLA Surface Temperature\n",
    study_start,
    " to ",
    study_end
  )
...
##################################
## STEP 11. SAVE COMPOSITE RASTERS
##################################
...
writeRaster(
  temperature_fahrenheit,
  "outputs/data/ucla_surface_temperature_median_2025.tif",
  overwrite = TRUE
)

writeRaster(
  ndvi,
  "outputs/data/ucla_ndvi_median_2025.tif",
  overwrite = TRUE
)

writeRaster(
  valid_observations,
  "outputs/data/ucla_valid_observations_2025.tif",
  overwrite = TRUE
)
...
##################################
## STEP 12. PREPARE BOUNDARY FOR MAPS
##################################

ucla_boundary_plot <- st_as_sf(
  ucla_boundary_vect
)


##################################
## STEP 13. POLISHED HEAT MAP
##################################
...
heat_map <- ggplot() +
  ...
  labs(
    title = "Land Surface Temperature Across UCLA",
    subtitle = paste0(
      "Median of ",
      nrow(scene_table),
...
observation_map <- ggplot() +
  ...
  labs(
    title = "Valid Satellite Observations Across UCLA",
    subtitle = paste0(
...
ggsave(
  filename = "outputs/figures/ucla_surface_temperature_composite.png",
  plot = heat_map,
  width = 8,
  height = 8,
  dpi = 300,
...
ggsave(
  filename = "outputs/figures/ucla_ndvi_composite.png",
  plot = ndvi_map,
  width = 8,
  height = 8,
  dpi = 300,
...
ggsave(
  filename = "outputs/figures/ucla_valid_observations.png",
...
cat(
  "Polished map images saved successfully!\n"
)


##################################
## STEP 17. PREPARE WEB MAP LAYERS
##################################
...
# Put all three layers on the same grid
ndvi_web <- resample(
  ndvi_web,
  heat_web,
  method = "bilinear"
...
##################################
## STEP 19. EXPORT WEB PNGS
##################################

png(
  filename = "web/data/ucla_heat.png",
  width = 1600,
  height = 1600,
  bg = "transparent"
)
...
png(
  filename = "web/data/ucla_ndvi.png",
  width = 1600,
  height = 1600,
  bg = "transparent"
)
...
cat(
  "Transparent UCLA web PNGs exported successfully!\n"
)


##################################
## STEP 20. CREATE WEB DATAFRAME
##################################
...
write.csv(
  analysis_df,
  "web/data/ucla_temperature_ndvi.csv",
  row.names = FALSE
)

cat(
  "UCLA web data CSV exported successfully!\n"
)


##################################
## STEP 21. EXPORT CAMPUS SUMMARY
##################################
...
##################################
## STEP 22. EXPORT UCLA METADATA
##################################

# Use the exact projected raster extent so
# the PNG aligns with the Leaflet image overlay
heat_extent <- ext(
  heat_web
)

ucla_map_metadata <- list(
  campus = "ucla",
  campus_name = "UCLA",
  statistic = "median",
  scene_count = nrow(scene_table),
  ...
  files = list(
    heat_image = "ucla_heat.png",
    ndvi_image = "ucla_ndvi.png",
    data = "ucla_temperature_ndvi.csv",
    summary = "ucla_summary.csv"
  )
)

write_json(
  ucla_map_metadata,
  "web/data/ucla_map_metadata.json",
  pretty = TRUE,
  auto_unbox = TRUE
)

print(ucla_map_metadata)

cat(
  "\nSaved web/data/ucla_map_metadata.json\n"
)


##################################
## STEP 23. CONFIRM OUTPUTS
##################################

cat(
  "\nAll UCLA outputs exported successfully!\n"
)
