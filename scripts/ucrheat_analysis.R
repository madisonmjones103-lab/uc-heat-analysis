...
if (!dir.exists(landsat_folder)) {
  ...
  stop(
    paste(
      "The UC Riverside Landsat folder does not exist:",
      landsat_folder
    )
  )
}
...
if (!file.exists(boundary_file)) {
  ...
  stop(
    paste(
      "The UC Riverside boundary file does not exist:",
      boundary_file
    )
  )
}
...
if (nrow(scene_table) == 0) {
  stop(
    "No complete UC Riverside Landsat scenes were matched."
  )
}
...
ucr_boundary <- st_read(
  boundary_file,
  quiet = TRUE
)

if (is.na(st_crs(ucr_boundary))) {
  stop(
    paste0(
      "The UC Riverside boundary does not have ",
...
ucr_boundary_projected <- st_transform(
  ucr_boundary,
  crs(reference_temperature)
)

ucr_boundary_vect <- vect(
  ucr_boundary_projected
)
...
plot(
  temperature_fahrenheit,
  main = paste0(
    "Median UC Riverside Surface Temperature\n",
    study_start,
    " to ",
    study_end
  )
...
cat(
  "Polished UC Riverside map images saved successfully!\n"
)
...
cat(
  "Transparent UC Riverside web PNGs exported successfully!\n"
)
...
cat(
  "UC Riverside web data CSV exported successfully!\n"
)
...
cat(
  "\nSaved web/data/ucr_map_metadata.json\n"
)
...
cat(
  "\nAll UC Riverside outputs exported successfully!\n"
)
...
cat(
  "\nUC Riverside files in outputs/data:\n"
)
...
cat(
  "\nUC Riverside files in outputs/figures:\n"
)
...
cat(
  "\nUC Riverside files in web/data:\n"
)
...
cat(
  "\nUC Riverside analysis complete!\n"
)