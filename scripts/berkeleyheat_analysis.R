...
##################################
## STEP 2. LOCATE BERKELEY FILES
##################################

campus <- "berkeley"
campus_name <- "UC Berkeley"

# Look for the Berkeley Landsat folder
# in several likely locations
landsat_candidates <- c(
  file.path(
    "landsat",
    "berkeleydata_landsat"
  ),
  file.path(
    "landsat",
    "berkeley_landsat"
...
if (!any(candidate_has_data)) {
  ...
  stop(
    paste0(
      "Could not find the Berkeley Landsat files.\n",
...
# Find the exact Berkeley boundary file
...
if (!any(boundary_exists)) {
  ...
  stop(
    paste0(
      "Could not find the Berkeley boundary file.\n",
...
# Extract satellite, path/row and date
...
if (nrow(scene_table) == 0) {
  stop(
    "No complete Berkeley Landsat scenes were matched."
  )
}
...
##################################
## STEP 5. IMPORT BERKELEY BOUNDARY
##################################

berkeley_boundary <- st_read(
  boundary_file,
  quiet = TRUE
)

if (is.na(st_crs(berkeley_boundary))) {
  stop(
    paste0(
      "The Berkeley boundary file does not ",
...
berkeley_boundary_projected <- st_transform(
  berkeley_boundary,
  crs(reference_temperature)
)

berkeley_boundary_vect <- vect(
  berkeley_boundary_projected
)
...
plot(
  temperature_fahrenheit,
  main = paste0(
    "Median Berkeley Surface Temperature\n",
    study_start,
    " to ",
    study_end
  )
...
berkeley_boundary_plot <- st_as_sf(
  berkeley_boundary_vect
)
...
cat(
  "Polished Berkeley map images saved successfully!\n"
)
...
cat(
  "Transparent Berkeley web PNGs exported successfully!\n"
)
...
cat(
  "Berkeley web data CSV exported successfully!\n"
)
...
##################################
## STEP 23. EXPORT BERKELEY METADATA
##################################
...
cat(
  "\nSaved web/data/berkeley_map_metadata.json\n"
)
...
cat(
  "\nAll Berkeley outputs exported successfully!\n"
)
...
cat(
  "\nBerkeley files in outputs/data:\n"
)
...
cat(
  "\nBerkeley files in outputs/figures:\n"
)
