...
if (!dir.exists(landsat_folder)) {
  ...
  stop(
    paste(
      "The UC Davis Landsat folder does not exist:",
      landsat_folder
    )
  )
}
...
if (!file.exists(boundary_file)) {
  ...
  stop(
    paste(
      "The UC Davis boundary file does not exist:",
      boundary_file
    )
  )
}
...
if (nrow(scene_table) == 0) {
  stop(
    "No complete UC Davis Landsat scenes were matched."
  )
}
...
##################################
## STEP 5. IMPORT UC DAVIS BOUNDARY
##################################

ucd_boundary <- st_read(
  boundary_file,
  quiet = TRUE
)

if (is.na(st_crs(ucd_boundary))) {
  stop(
    paste0(
      "The UC Davis boundary does not have ",
...
ucd_boundary_projected <- st_transform(
  ucd_boundary,
  crs(reference_temperature)
)

ucd_boundary_vect <- vect(
  ucd_boundary_projected
)
...
plot(
  temperature_fahrenheit,
  main = paste0(
    "Median UC Davis Surface Temperature\n",
    study_start,
    " to ",
    study_end
  )
...
cat(
  "Polished UC Davis map images saved successfully!\n"
)
...
cat(
  "Transparent UC Davis web PNGs exported successfully!\n"
)
...
cat(
  "UC Davis web data CSV exported successfully!\n"
)
...
##################################
## STEP 23. EXPORT UC DAVIS METADATA
##################################
...
cat(
  "\nSaved web/data/ucd_map_metadata.json\n"
)
...
cat(
  "\nAll UC Davis outputs exported successfully!\n"
)
...
cat(
  "\nUC Davis files in outputs/data:\n"
)
...
cat(
  "\nUC Davis files in outputs/figures:\n"
)
