...
##################################
## STEP 2. LOCATE UC SANTA BARBARA FILES
##################################

campus <- "ucsb"
campus_name <- "UC Santa Barbara"

# Look for the UC Santa Barbara Landsat folder
...
if (!any(candidate_has_data)) {
  ...
  stop(
    paste0(
      "Could not find the UC Santa Barbara Landsat files.\n",
...
# Find the exact UC Santa Barbara boundary file
...
if (!any(boundary_exists)) {
  ...
  stop(
    paste0(
      "Could not find the UC Santa Barbara boundary file.\n",
...
if (nrow(scene_table) == 0) {
  stop(
    "No complete UC Santa Barbara Landsat scenes were matched."
  )
}
...
##################################
## STEP 5. IMPORT UC SANTA BARBARA BOUNDARY
##################################

ucsb_boundary <- st_read(
  boundary_file,
  quiet = TRUE
)

if (is.na(st_crs(ucsb_boundary))) {
  stop(
    paste0(
      "The UC Santa Barbara boundary file does not ",
...
ucsb_boundary_projected <- st_transform(
  ucsb_boundary,
  crs(reference_temperature)
)

ucsb_boundary_vect <- vect(
  ucsb_boundary_projected
)
...
plot(
  temperature_fahrenheit,
  main = paste0(
    "Median UC Santa Barbara Surface Temperature\n",
    study_start,
    " to ",
    study_end
  )
...
cat(
  "Polished UC Santa Barbara map images saved successfully!\n"
)
...
cat(
  "Transparent UC Santa Barbara web PNGs exported successfully!\n"
)
...
cat(
  "UC Santa Barbara web data CSV exported successfully!\n"
)
...
##################################
## STEP 23. EXPORT UC SANTA BARBARA METADATA
##################################
...
cat(
  "\nSaved web/data/ucsb_map_metadata.json\n"
)
...
cat(
  "\nAll UC Santa Barbara outputs exported successfully!\n"
)
...
cat(
  "\nUC Santa Barbara files in outputs/data:\n"
)
...
cat(
  "\nUC Santa Barbara files in outputs/figures:\n"
)
...
cat(
  "\nUC Santa Barbara files in web/data:\n"
)
...
cat(
  "\nUC Santa Barbara analysis complete!\n"
)